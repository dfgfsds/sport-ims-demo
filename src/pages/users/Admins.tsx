...
const Admins: React.FC = () => {
  ...
  const handleDeleteAdmin = async (adminId: string) => {
    ...
    try {
      await axios.delete(`${baseURL}/admins/${adminId}`);
  ...
  const handleSaveAdmin = async (e: React.FormEvent) => {
    ...
    if (modalMode === 'create') {
      ...
      await axios.post(`${baseURL}/admins/`, payload);
      ...
      await axios.put(`${baseURL}/admins/${selectedAdmin.id}`, payload);
    }
          ...
          {/* {modalMode !== 'view' && (
            <div className="flex items-center">
              ...
              <label htmlFor="approved" className="ml-2 text-sm text-gray-700">
                Approve (admin:any)registration
              </label>
            </div>
          )} */}
