...
const ClubRegistrations: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDistricts = async () => {
      try {
        const res = await axios.get(`${API_BASE_URL}/districts/?stateId=${stateId}`);
  ...
  }, [selectedEventId,selectedDistrictId]);

const fetchClubRegistrations = async () => {
      try {
      const res = await axios.get(
        `${API_BASE_URL}/registrations/count-by-club-state/${selectedEventId}/${stateId}/?districtId=${selectedDistrictId}`
      );
      setClubRegistrations(res.data || []);
      } catch {
      setClubRegistrations([]);
      } finally {
      setLoading(false);
