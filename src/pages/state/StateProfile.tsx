...
const ClubProfile: any = ({ stateId }: any) => {
  const [isEditing, setIsEditing] = React.useState(false);
  const [pendingChanges, setPendingChanges] = React.useState(false);
  const [profileData, setProfileData] = useState<any>('')
  const baseUrl = import.meta.env.VITE_API_BASE_URL

  const stateSecretaryId =localStorage.getItem('userId')

  useEffect(() => {
    const fetchClubProfile = async () => {
      try {
        const response = await axios.get(
          `${baseUrl}/state_secretaries/string/${stateSecretaryId}`
        );
  ...
  const handleSave = async () => {
    try {
      const updateApi = await axios.put(`${baseUrl}/state_secretaries/${stateSecretaryId}`, profileData);
      if (updateApi) {
        setIsEditing(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">State Profile</h1>
