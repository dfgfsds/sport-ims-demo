...
const StateLayout: React.FC = () => {
  const [sidebarOpen, setSidebarOpen] = React.useState(false);
  const navigate = useNavigate();
  const [showLogoutModal, setShowLogoutModal] = useState(false);

  const menuItems = [
    // { icon: LayoutDashboard, label: 'Dashboard', path: '/organiser' },
    { icon: Calendar, label: 'District', path: '/organiser/district' },
    { icon: Users, label: 'Clubs', path: '/organiser/club' },
    // { icon: Trophy, label: 'Past Events Report', path: '/organiser/reports' },
    // { icon: Building2, label: 'State Profile', path: '/organiser/profile' }
  ];
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        } transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0`}>

        {/* Sidebar header */}
        <div className="flex items-center justify-between p-4 border-b border-slate-700">
          <div>
            <h1 className="text-lg font-bold text-blue-400">Event Organizer Portal</h1>
