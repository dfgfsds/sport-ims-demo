...
const DistrictLayout: React.FC = () => {
  const [sidebarOpen, setSidebarOpen] = React.useState(false);
  const navigate = useNavigate();
  const districtName = localStorage.getItem("districtName");
  const userId = localStorage.getItem("userId");
  const secretaryName = localStorage.getItem("secretaryName");
  const [showLogoutModal, setShowLogoutModal] = useState(false);

  const menuItems = [
    { icon: LayoutDashboard, label: 'Dashboard', path: '/district' },
    { icon: Users, label: 'Clubs', path: '/district/club' },
    { icon: Calendar, label: 'Events', path: '/district/events' },
    // { icon: Trophy, label: 'Past Events Report', path: '/district/reports' },
    { icon: Building2, label: 'District Profile', path: '/district/profile' }
  ];

  const currentState: any = {
    name: districtName,
    code: userId,
    contactPerson: secretaryName,
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700">
          <div>
            <h1 className="text-lg font-bold text-blue-400">District Portal</h1>
