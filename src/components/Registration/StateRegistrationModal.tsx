...
//     try {
//       // Simulate API call
//       const updateApi = await axios.post(`${baseUrl}/state_secretaries/register`, formData);
//       if (updateApi) {
//         onClose();
//         setFormData('')
//       }
...
const StateRegistrationModal: React.FC<StateRegistrationModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await axios.put(`${baseUrl}/state_secretaries/register`, formData);
      if (res) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField label="Secretary Name" required>
            <input
              ...
              placeholder="Enter state secretary name"
              required
            />
          </FormField>
